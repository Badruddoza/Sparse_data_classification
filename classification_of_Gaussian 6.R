...
#Analysis
data1=cbind.data.frame(xs,malignant1,seq(1,n,1))
names(data1)[1:T] <- unlist(mapply(function(x,y) paste(x, seq(1,y), sep=""), x="X", y=T))
col_temp1<-T+1;col_temp2<-dim(data1)[2]
names(data1)[col_temp1:col_temp2]<-c("m","id")
head(data1)


#Add missing points in the data
xs_m<-xs
pc_missing=10 #percentage of data missing
missing_token<-matrix(c(runif(n*T)),nrow=n,ncol=T)
for(i in 1:n){
  for(j in 1:T){
    if(missing_token[i,j]<pc_missing/100){
      xs_m[i,j]<-NA 
    }
  }
}
...
data2=cbind.data.frame(xs_m,m=data1$m,id=data1$id)
names(data2)[1:T] <- unlist(mapply(function(x,y) paste(x, seq(1,y), sep=""), x="X", y=T))
head(data2,4)
...
interpolated<-c()
for(i in 1:n){
  interpolated<-c(data3,na.approx(unlist(data2[i,1:T]),t))
}
matrix(c(interpolated),nrow=n,ncol=T,byrow=TRUE)


f<-approx(t,yy,method = "constant")
plot(f$x,f$y)
f
